...
/*   Updated: 2019/09/23 08:28:08 by blukasho         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		check_errors_get_rooms(t_lemin *lemin)
{
	if (!lemin->start_room)
...
char			*get_rooms(t_lemin *lemin)
{
	...
	while (!errno && (input = lemin_get_line()))
	{
		if (!errno && (ISCOMMENT(input) || (ISCOMMAND(input) && !ISSTART(input)
			&& !ISEND(input))))
			ft_strdel(&input);
		else if (!errno && ISCOMMAND(input) && ISSTART(input) &&
				!ft_strdel(&input))
			add_room(lemin, NULL, STARTROOM);
		else if (!errno && ISCOMMAND(input) && ISEND(input) &&
				!ft_strdel(&input))
			add_room(lemin, NULL, ENDROOM);
		else if (!errno && !ISLINK(input))
			add_room(lemin, input, DEFAULTROOM);
		else if (!errno && !check_errors_get_rooms(lemin))
			return (input);
		if (input)
			ft_strdel(&input);
	}
